...
with open("{}/.config/qtile/config/settings.json".format(
        os.getenv("HOME"))) as file:
    settings = json.load(file)

looks: dict = settings["looks"]
display: dict = settings["display"]

with open("{}/.config/qtile/config/colors.json".format(
        os.getenv("HOME"))) as file:
...
keys = [
    ...
    Key([mod],
        ...
        desc="Show active windows in rofi"),
    Key([mod], "f", lazy.spawn("flameshot gui"), desc="Open flameshot gui"),
    Key([mod],
        "s",
        lazy.spawn("scrot"),
        desc="Take full screen ss using scrot"),
    Key([mod],
        "d",
...
widgets_list: list = [
    ...
    widget.Prompt(foreground=colors["active"],
                  background=colors["groups_bg"],
                  font="Jetbrains Mono",
                  prompt="Woof: "),
    widget.Sep(padding=6, linewidth=0, background=colors["seperator"]),
